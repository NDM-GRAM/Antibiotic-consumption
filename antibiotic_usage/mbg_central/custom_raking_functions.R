...
###########################################################
#### Save outputs from custom raking function
...
#'          "draws": data.table with rows for each aggregated area & year,
#'                   and columns representing [field], year, and draws[V1...Vn]
#'          "shapefile_data": contents of the shapefile defined by `shapefile_path`,
#'                            which can be merged on to "draws" to add on admin names, etc
#'          "summarized": data table with row for each aggregated area & year,
#'                        and columns for [field], year, and all summary stats in `ss`
#'                        (if ss=NULL, then this is also NULL)
#'          "shapefile": the shapefile itself (if return_shapefile = T; otherwise will be NULL)
#'
#' @export N/A
#'
#' @examples
...
#' @export
#'
#' @examples custom_admin_raster <- load_custom_admin_raster(shapefile_path, field, simple_raster)
#' 
