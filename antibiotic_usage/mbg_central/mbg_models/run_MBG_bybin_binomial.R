...
if(keep_inla_files==FALSE) inla_working_dir <- '/snfs1/temp/geospatial/inla_intermediate/inla_scratch'
if(keep_inla_files==TRUE) inla_working_dir <- paste0('/snfs1/temp/geospatial/inla_intermediate/inla_', run_date)
system.time(
  res_fit <- inla(f_mbg,
...
inla_working_dirs <- list.dirs('/snfs1/temp/geospatial/inla_intermediate', recursive = FALSE)
...
# dummy raster
template <- raster(paste0('/share/geospatial/mbg/', indicator_group, '/', indicator, '/templates/template5kdata'))
...
# extract
covs <- brick(paste0('/share/geospatial/mbg/', indicator_group, '/', indicator, '/covs/covs_transformed.grd')) # non-varying
