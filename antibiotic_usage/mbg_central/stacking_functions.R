...
run_child_stackers <- function(models, input_data = the_data) {
  ...
  if('nnet' %in% models){
    ...
    #Define the formula
    form <- as.formula(paste0('rate ~ ', fixed_effects))
    ...
    train_control <- trainControl(selectionFunction = "best",
                                  ...
                                  number = 5,
                                  repeats = 5,
                                  index = list(input_data$a_rowid[input_data$fold_id!=1],
                                               input_data$a_rowid[input_data$fold_id!=2],
                                               input_data$a_rowid[input_data$fold_id!=3],
                                               input_data$a_rowid[input_data$fold_id!=4],
                                               input_data$a_rowid[input_data$fold_id!=5]),
                                  indexOut =list(input_data$a_rowid[input_data$fold_id==1],
                                                 input_data$a_rowid[input_data$fold_id==2],
                                                 input_data$a_rowid[input_data$fold_id==3],
                                                 input_data$a_rowid[input_data$fold_id==4],
                                                 input_data$a_rowid[input_data$fold_id==5]),
    ...
    # Extract out of sample and in sample predictions
    input_data[, 'nnet_cv_pred'   := arrange(nn_fit_final$pred, rowIndex)[,"pred"]]
    input_data[, 'nnet_full_pred' := predict(nn_fit_final, input_data)]
    
    nnet_child <- list(dataset = input_data[, c('nnet_cv_pred', 'nnet_full_pred')],
                         nnet = nn_fit_final)
    input_data$nnet_cv_pred <-  NULL
    input_data$nnet_full_pred <-  NULL
    the_data$nnet_cv_pred <-  NULL
    the_data$nnet_full_pred <-  NULL
  ...
  if('cubist' %in% models){
    ...
    # Extract out of sample and in sample predictions
    input_data[, 'cubist_cv_pred'   := arrange(cubist_fit_final$pred, rowIndex)[,"pred"]]
    input_data[, 'cubist_full_pred' := predict(cubist_fit_final, input_data)]
    toc(log = T)
    
    cubist_child <- list(dataset = input_data[, c('cubist_cv_pred', 'cubist_full_pred')],
                         cubist = cubist_fit_final)
    
    input_data$cubist_cv_pred <-  NULL
    input_data$cubist_full_pred <-  NULL
