...
#'
#' @param id_raster The id raster used for building the link table, if this is null the default is the empty_world_raster() which 
#'                  is used for most cases that are not custom shapefiles 
#'
...
#' @export
#'
rasterize_check_coverage <- function(shapes, template_raster, field, ..., link_table = NULL, id_raster = NULL) {
  ...
  if (is.null(id_raster)) {
    ...
    reference_pixel_owner <- suppressWarnings(empty_world_raster()) 
  } else {
...
build_simple_raster_pop <- function(subset_shape,
  ...
  # To ensure correct "snap" method is used, first convert to a template raster before loading population 
  template_raster <- raster::crop(empty_world_raster(whole_world = T), raster::extent(subset_shape), snap = "out")
...
pull_custom_modeling_regions <- function(custom_regions){
  ...
  ref_reg_list <- list(
    ...
    #add regions for anitbiotic use
    'balkans_ext' = 'alb+blr+bih+hrv+mkd+mda+mne+rou+srb+ukr+bgr+xko',
    'caucasus' = 'arm+aze+geo+tur'
